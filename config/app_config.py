...
class BaseConfig:
    ...
    # ============ RAG 向量信息库配置（PostgreSQL + pgvector） ============
    # 用于文档向量化存储和语义检索，独立于业务 MySQL 数据库
    ...
    @classmethod
    def get_rag_pg_config_dict(cls):
        """获取 RAG 向量信息库（PostgreSQL）配置字典"""
        return {
            'host': cls.RAG_PG_HOST,
            'port': cls.RAG_PG_PORT,
            'user': cls.RAG_PG_USER,
            'password': cls.RAG_PG_PASSWORD,
            'dbname': cls.RAG_PG_DATABASE,
        }
