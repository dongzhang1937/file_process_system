        ...
        if file_path.endswith('.docx'):
            ...
            try:
                ...
                for c in chapters:
                    ...
                    """
                    # 限制 content 长度，避免数据库字段溢出（假设 TEXT 字段最大 65535 字节）
                    content_value = c['content']
                    if content_value and len(content_value) > 60000:
                        content_value = content_value[:60000] + '\n...[内容已截断]'
                        logger.warning(f"章节内容过长已截断: title={c['title'][:50]}, original_len={len(c['content'])}")
                    
                    # 限制 title 长度
                    title_value = c['title']
                    if title_value and len(title_value) > 500:
                        title_value = title_value[:500]
                    
                    params = (
                        doc_id, parent_db_id, c['level'], order_index, title_value, content_value,
            ...
            if result.get('status') == 'success':
                ...
                # 执行文档向量化（用于语义搜索）— 使用 pgvector RAG 方案
                embedding_info = {
                    'status': 'skipped',
                    'embed_count': 0,
                    'errors': []
                ...
                try:
                    ...
                    if rag_result.get('success'):
                        embedding_info = {
                            'status': 'success',
                            'embed_count': rag_result.get('chunks', 0),
                            'sections': rag_result.get('sections', 0),
                            'rag_document_id': rag_result.get('document_id'),
                            'errors': []
                        }
                        logger.info(f"文档 RAG 向量化完成: doc_id={doc_id}, "
                                   f"sections={rag_result.get('sections')}, chunks={rag_result.get('chunks')}")
                    else:
                        embedding_info = {
                            'status': 'failed',
                            'embed_count': 0,
                            'errors': [rag_result.get('message', '未知错误')]
                        }
                        logger.warning(f"文档 RAG 向量化失败: {rag_result.get('message')}")
                except Exception as embed_error:
                    # 向量化失败不影响主流程，但记录错误
                    embedding_info = {
                        'status': 'failed',
                        'embed_count': 0,
                        'errors': [str(embed_error)]
                    }
                    logger.warning(f"文档向量化失败（不影响主流程）: {embed_error}")
                
                # 更新处理结果，包含向量化信息
                process_result['embedding'] = embedding_info
                
                # 重新保存包含向量化信息的处理结果
                update_sql = """
                    UPDATE doc_process_records
                    SET process_result = %s
                    WHERE doc_id = %s
                """
                task_dml_sql(update_sql, parameters=(json.dumps(process_result, ensure_ascii=False), doc_id))
