...
# 2. 初始化扩展
    ...
    from .models.embedding_routes import embedding_bp  # Embedding配置管理蓝图
    from .models.rag_routes import rag_bp  # RAG向量检索蓝图
    ...
    app.register_blueprint(embedding_bp)  # 注册Embedding配置蓝图
    app.register_blueprint(rag_bp)  # 注册RAG向量检索蓝图
    ...
    # 应用启动时清理卡住的后台分析任务（running > 2小时 → failed）
    try:
        with app.app_context():
            from .models.analysis_tasks import AnalysisTaskManager
            AnalysisTaskManager.cleanup_stale_tasks()
            logger.info("应用启动时分析任务清理检查完成")
    except Exception as e:
        logger.error(f"应用启动时分析任务清理失败: {e}")
